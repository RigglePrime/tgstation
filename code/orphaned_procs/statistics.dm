/proc/sql_poll_players()
	...
	if(!SSdbcore.IsConnected())
		...
		var/datum/db_query/query = SSdbcore.NewQuery("INSERT INTO [format_table_name("legacy_population")] (playercount, time) VALUES ([playercount], '[sqltime]')")
		if(!query.Execute())
...
/proc/sql_poll_admins()
	...
	if(!SSdbcore.IsConnected())
		...
		var/sqltime = time2text(world.realtime, "YYYY-MM-DD hh:mm:ss")
		var/datum/db_query/query = SSdbcore.NewQuery("INSERT INTO [format_table_name("legacy_population")] (admincount, time) VALUES ([admincount], '[sqltime]')")
		if(!query.Execute())
			var/err = query.ErrorMsg()
...
/proc/sql_commit_feedback()
	...
	if(!SSdbcore.IsConnected())
		...
		var/datum/db_query/max_query = SSdbcore.NewQuery("SELECT MAX(roundid) AS max_round_id FROM [format_table_name("feedback")]")
		max_query.Execute()
		...
		for(var/datum/feedback_variable/item in content)
			...
			var/datum/db_query/query = SSdbcore.NewQuery("INSERT INTO [format_table_name("feedback")] (id, roundid, time, variable, value) VALUES (null, [newroundid], Now(), '[variable]', '[value]')")
			if(!query.Execute())
				var/err = query.ErrorMsg()
