...
/client/proc/admin_memo_output(task)
	...
	switch(task)
		if("Write")
			var/datum/db_query/query_memocheck = SSdbcore.NewQuery("SELECT ckey FROM [format_table_name("memo")] WHERE ckey = '[sql_ckey]'")
			...
			var/datum/db_query/query_memoadd = SSdbcore.NewQuery("INSERT INTO [format_table_name("memo")] (ckey, memotext, timestamp) VALUES ('[sql_ckey]', '[memotext]', '[timestamp]')")
		...
		if("Edit")
			...
			var/datum/db_query/query_memofind = SSdbcore.NewQuery("SELECT memotext FROM [format_table_name("memo")] WHERE ckey = '[target_sql_ckey]'")
			...
			if(query_memofind.NextRow())
				...
				var/datum/db_query/update_query = SSdbcore.NewQuery("UPDATE [format_table_name("memo")] SET memotext = '[new_memo]', last_editor = '[sql_ckey]', edits = CONCAT(IFNULL(edits,''),'[edit_text]') WHERE ckey = '[target_sql_ckey]'")
		...
		if("Remove")
			...
			if(!target_ckey)
				return
			var/target_sql_ckey = sanitizeSQL(target_ckey)
			var/datum/db_query/query_memodel = SSdbcore.NewQuery("DELETE FROM [format_table_name("memo")] WHERE ckey = '[target_sql_ckey]'")
